...
hyper_parameters = {'learning_rate_top_model': 0.001, 'learning_rate_organization_model': 0.0005, 'batch_size': 512}
dataset = 'MNIST' 
...
def get_optimizer(model_params, optimizer_name, learning_rate):
    if optimizer_name == 'SGD':
        return torch.optim.SGD(model_params, lr=learning_rate)
    elif optimizer_name == 'Adam':
        return torch.optim.Adam(model_params, lr=learning_rate)
    elif optimizer_name == 'RMSprop':
        return torch.optim.RMSprop(model_params, lr=learning_rate)
    elif optimizer_name == 'AdaGrad':
        return torch.optim.Adagrad(model_params, lr=learning_rate)
    elif optimizer_name == 'AdaDelta':
        return torch.optim.Adadelta(model_params, lr=learning_rate)
    else:
        raise ValueError(f"Unknown optimizer: {optimizer_name}")
    
def apply_dropout_after_epochs(active_clients, current_epoch, start_dropout_epoch):
    if current_epoch >= start_dropout_epoch:
        # Drop the last client by setting its value to False
        active_clients[-1] = False
...
#     """
#     Save results to a CSV file. Append if the file exists; otherwise, create a new file.
...
class Vertical_FL_Train:
	...
	def run(self, args):
		...
		if model_type == 'vertical':
			...
			optimizer = get_optimizer(top_model.parameters(), args.top_model_optimizer, learning_rates[0])
			print(f"optimizer is : {optimizer}")
			optimizer_organization_list = []
			for organization_idx in range(organization_num):
				optimizer_organization_list.append(
         		   get_optimizer(organization_models[organization_idx].parameters(), args.organization_model_optimizer, learning_rates[1]))
			...
			for i in range(epochs):
				print('Epoch: ', i)
				# Apply dropout after specified epochs
				self.active_clients = apply_dropout_after_epochs(
					self.active_clients, current_epoch=i, start_dropout_epoch=start_dropout_epoch
				)
				print(f"Active clients at epoch {i}: {self.active_clients}")
...
if __name__ == "__main__":
	...
	parser.add_argument('--optimizer', default='Adam', help='Optimizer for the top model (e.g., SGD, Adam, RMSprop, etc.)')	
	parser.add_argument('--top_model_optimizer', default='Adam', help='Optimizer for the top model (e.g., SGD, Adam, RMSprop, etc.)')
	parser.add_argument('--organization_model_optimizer', default='Adam', help='Optimizer for organization models (e.g., SGD, Adam, RMSprop, etc.)')
	
	args = parser.parse_args()
	args.top_model_optimizer = args.optimizer
	args.organization_model_optimizer = args.optimizer
	...
	print("test_acc_array=",test_acc_array)
 
	# Plotting Lo`ss and Accuracy over Epochs
	epochs_range = range(1, num_default_epochs + 1)

	# Plot Train and Validation Loss
	plt.figure(figsize=(12, 6))
	plt.plot(epochs_range, train_loss_array_avg, label="Train Loss")
	plt.plot(epochs_range, val_loss_array_avg, label="Validation Loss")
	plt.title("Training and Validation Loss")
	plt.xlabel("Epochs")
	plt.ylabel("Loss")
	plt.legend()
	plt.grid(True)
	plt.savefig(f"./images/loss_plot_{args.optimizer}.png")
	print("Loss plot saved")

	# Plot Train and Validation Accuracy
	plt.figure(figsize=(12, 6))
	plt.plot(epochs_range, train_auc_array_avg, label="Train Accuracy")
	plt.plot(epochs_range, val_auc_array_avg, label="Validation Accuracy")
	plt.title("Training and Validation Accuracy")
	plt.xlabel("Epochs")
	plt.ylabel("Accuracy")
	plt.legend()
	plt.grid(True)
	plt.savefig(f"./images/accuracy_plot_{args.optimizer}.png")
	print("Accuracy plot saved")



output_filename = f"./result/results_{args.optimizer}.txt"

# Write the information to a text file
with open(output_filename, 'w') as file:
    file.write(f"num_epochs = {num_default_epochs}\n")
    file.write(f"train_loss_array = {list(train_loss_array_avg)}\n")
    file.write(f"val_loss_array = {list(val_loss_array_avg)}\n")
    file.write(f"train_accuracy_array = {list(train_auc_array_avg)}\n")
    file.write(f"val_accuracy_array = {list(val_auc_array_avg)}\n")
    file.write(f"test_acc_avg = {test_acc_avg}\n")
    file.write(f"test_acc_array = {test_acc_array}\n")
